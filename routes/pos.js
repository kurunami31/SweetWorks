...
router.get('/', (req, res) => {
  ...
  const customers = db.query("SELECT * FROM customers ORDER BY name");
...
router.post('/checkout', (req, res) => {
  ...
  const orderId = db.run("INSERT INTO orders (customer_id, order_type, status, total, notes) VALUES (?, 'pos', 'completed', ?, ?)",
    [cid, total, payment || '']);
...
router.post('/customer', (req, res) => {
  ...
  const id = db.run("INSERT INTO customers (name, email, phone) VALUES (?, ?, ?)", [name, email || '', phone || '']);
